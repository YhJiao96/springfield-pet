...
def main(argv=None):
    ...
    ap.add_argument("--assets", type=Path,
                    default=Path(__file__).resolve().parent.parent / "assets" / "pet_assets",
                    help="pet_assets 目录")
    ...
    if not args.source.is_dir():
        ap.error(f"找不到目录: {args.source}")
    if not re.fullmatch(r"[A-Za-z0-9_-]+", args.name):
        ap.error("--name 只能用字母/数字/下划线/连字符")

    manifest_path = args.assets / "manifest.json"
    if manifest_path.exists() and not args.force:
        existing = json.loads(manifest_path.read_text(encoding="utf-8"))
        if args.name in existing.get("skins", {}):
            ap.error(f"皮肤 {args.name} 已存在,要覆盖请加 --force")

    inputs, unknown = collect_inputs(args.source)
    if unknown:
        print("跳过(认不出是哪个动作):")
        for n in unknown:
            print(f"    {n}")
    if not inputs:
        ap.error("目录里没有能识别的动作文件。文件名请用 wait/move/victory… 见 --help")
    if not any(slot == "wait" for _p, slot in inputs):
        print("⚠️  没有 wait(待机)动作 —— 桌宠会拿第一个动作凑合,建议补一个 wait.gif")

    print(f"\n导入皮肤 {args.name}({len(inputs)} 个动作):")
    animations = {}
    for path, slot in inputs:
        images, anchor = process_one(path, slot, args)
        animations[slot] = build_entry(slot, len(images), images[0].size, anchor, args.name)
        if not args.dry_run:
            write_frames(images, args.assets / args.name / slot)

    if args.dry_run:
        print("\n--dry-run:没有写任何文件。manifest 记录会是:")
        print(json.dumps({args.name: {"animations": animations}}, ensure_ascii=False, indent=2))
        return 0

    merge_manifest(manifest_path, args.name, animations,
                   args.display or args.name, args.fps)
    total = sum(a["frames"] for a in animations.values())
